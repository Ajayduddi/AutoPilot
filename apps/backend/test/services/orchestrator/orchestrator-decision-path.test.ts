...
import { __orchestratorTestUtils, OrchestratorService } from "../../../src/services/orchestrator/orchestrator.service";
import { ChatService } from "../../../src/services/chat.service";
import { ChatRepo } from "../../../src/repositories/chat.repo";
import { ContextService } from "../../../src/services/context/context.service";
import { MainAgentService } from "../../../src/services/ai-routing/main-agent.service";
...
describe("OrchestratorService decision-path unit coverage", () => {
  it("upgrades broader natural choice endings into interactive questions", () => {
    const question = __orchestratorTestUtils.buildQuestionFromText(
      "I can help further. Would you like a resume tweak, LinkedIn optimization tip, or both?",
    ) as any;

    expect(question?.type).toBe("question_mcq");
    expect(question?.prompt).toBe("Choose how you want to continue:");
    expect(question?.options?.length).toBe(3);
    expect(question?.options?.[0]?.valueToSend).toBe("resume tweak");
    expect(question?.options?.[1]?.valueToSend).toBe("LinkedIn optimization tip");
    expect(question?.options?.[2]?.valueToSend).toBe("both");
  });

  it("strips upgraded trailing natural choice prompts from visible markdown", () => {
    const visible = __orchestratorTestUtils.stripTrailingQuestionPrompt(
      "You have a strong profile overall.\nWould you like a resume tweak or LinkedIn optimization tip?",
    );

    expect(visible).toBe("You have a strong profile overall.");
  });

  it("formats common review-style markdown into cleaner section spacing", () => {
    const formatted = __orchestratorTestUtils.formatCommonAnswerMarkdown([
    ...
    expect(formatted).toContain("Profile Rating: 9.2/10\n\nKey Strengths (9.2/10)");
    expect(formatted).toContain("Areas for Improvement (8/10)\n\n- Open-Source Contributions - Limited public activity.");
  });
