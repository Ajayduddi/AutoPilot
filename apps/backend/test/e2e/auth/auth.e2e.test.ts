...
describe("Auth API Endpoints (/api/auth)", () => {
  it("POST /onboarding/register - rejects weak passwords", async () => {
    const app = buildApp();
    app.use("/api/auth", authRouter);

    const server = await withServer(app);
    try {
      const res = await fetch(`${server.baseUrl}/api/auth/onboarding/register`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ email: "u@test.com", password: "short123" }),
      });
      expect(res.status).toBe(400);
      const json = await res.json() as any;
      expect(json.error).toBe("Password must be at least 12 characters.");
    } finally {
      await server.close();
    }
  });
  ...
  it("POST /login - returns 202 when TOTP MFA is enabled", async () => {
    const app = buildApp();
    app.use("/api/auth", authRouter);

    (UserRepo as any).getByEmail = async () => ({
      id: "usr_1",
      email: "u@test.com",
      passwordHash: "hash:ok",
      name: "U",
      mfaTotpSecretEnc: "mfa:v1:encrypted",
    ...
    try {
      const res = await fetch(`${server.baseUrl}/api/auth/login`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ email: "u@test.com", password: "password1234" }),
      });
      expect(res.status).toBe(202);
      const json = await res.json() as any;
      expect(json.data.mfaRequired).toBe(true);
      expect(json.data.method).toBe("totp");
    } finally {
      await server.close();
    }
  });

  it("POST /mfa/totp/verify - completes pending MFA session", async () => {
    const app = buildApp();
    app.use("/api/auth", authRouter);

    (AuthService as any).verifyTotpForPendingSession = async () => ({
      ok: true,
      user: { id: "usr_1", email: "u@test.com", name: "U" },
    });

    const server = await withServer(app);
    try {
      const res = await fetch(`${server.baseUrl}/api/auth/mfa/totp/verify`, {
        method: "POST",
        headers: {
          "content-type": "application/json",
          "cookie": "ap_session=pending",
        },
