...
function getCallbackBaseUrl() {
  return getRuntimeConfig().callbackBaseUrl;
}
...
export class WorkflowService {
  private static scheduleWorkflowEmbeddingIndex(workflow: any) {
    EmbeddingIndexService.indexWorkflowDefinition({
      workflowId: workflow.id,
      userId: workflow.ownerUserId ?? null,
      key: workflow.key,
      name: workflow.name,
      description: workflow.description || null,
      provider: workflow.provider,
      tags: (workflow.tags as string[] | null) || [],
      metadata: (workflow.metadata as Record<string, unknown> | null) || null,
    }).catch((err) => {
      logger.warn({ scope: 'workflow.service', message: 'Workflow embedding indexing failed', err });
    });
  }

  private static scheduleWorkflowRunEmbeddingIndex(input: {
    runId: string;
    workflowId: string;
    workflowKey: string;
    workflowName?: string;
    ...
    EmbeddingIndexService.indexWorkflowRun({
      runId: input.runId,
      ...
      logger.warn({ scope: 'workflow.service', message: 'Workflow run embedding indexing failed', err });
    });
  }
  ...
  // ── CRUD ─────────────────────────────────────────────────────
    ...
    static async create(data: Omit<CreateWorkflowInput, 'id'>) {
        const workflow = await WorkflowRepo.createWorkflow({ ...data, id: crypto.randomUUID() });
    this.scheduleWorkflowEmbeddingIndex(workflow);
  ...
  static async execute(
    ...
    this.dispatchViaAdapter(runId, workflowId, workflowKey, provider as WorkflowProvider, userId, traceId, triggerSource, payload, threadId);
  ...
  static async executeAndAwait(
    ...
    await this.dispatchViaAdapter(
      ...
      userId, traceId, triggerSource, payload, threadId,
    );
  ...
  private static async dispatchViaAdapter(
    ...
    try {
      ...
      // Build standard execution request
        ...
        input: payload || {},
        callbackUrl: `${getCallbackBaseUrl()}/api/webhooks/callback`,
        meta: { runId, threadId },
      };
      ...
      if (result.status === 'failed') {
        ...
        }).catch(err => logger.error({ scope: 'workflow.service', message: 'Context indexing failed', err }));

        this.scheduleWorkflowRunEmbeddingIndex({
          runId,
          workflowId,
          workflowKey,
          workflowName: workflow.name,
          ...
          threadId,
          errorSummary: result.error?.error as string || result.error?.message as string || 'Unknown error',
          executionNotes: ['Provider returned failed status.'],
        });
      } else {
        ...
        this.scheduleWorkflowRunEmbeddingIndex({
          ...
          userId,
          threadId,
          resultSummary: (result.result?.summary as string | undefined) || null,
          resultData: (result.result?.data as Record<string, unknown> | undefined) || (result.result as Record<string, unknown> | null),
          executionNotes: ['Provider execution completed successfully.'],
        });
      }
      ...
      logger.error({ scope: 'workflow.service', message: 'Workflow dispatch failed', runId, err });
      ...
      this.scheduleWorkflowRunEmbeddingIndex({
        ...
        threadId,
        errorSummary: errorPayload.error,
        executionNotes: ['Dispatch failed before provider completion.'],
      });
      if (threadId) {
        ContextService.indexAuditEvent({
          threadId,
  ...
  static async updateRunStatus(
    ...
    if (run && (status === 'completed' || status === 'failed')) {
      const workflow = await WorkflowRepo.getWorkflowById(run.workflowId).catch(() => null);
      const output = (run.normalizedOutput && typeof run.normalizedOutput === 'object')
        ? (run.normalizedOutput as Record<string, unknown>)
        : {};
      const outputData = (output.data && typeof output.data === 'object')
        ? (output.data as Record<string, unknown>)
        : output;
      const error = (run.errorPayload && typeof run.errorPayload === 'object')
        ? (run.errorPayload as Record<string, unknown>)
        : {};
      ...
      this.scheduleWorkflowRunEmbeddingIndex({
        ...
        workflowName: workflow?.name || run.workflowKey,
        provider: run.provider,
        status: run.status,
        triggerSource: run.triggerSource,
        userId: run.userId,
        threadId: run.threadId || undefined,
        resultSummary: (output.summary as string | undefined) || null,
        resultData: outputData,
        errorSummary: (error.error as string | undefined) || (error.message as string | undefined) || null,
        executionNotes: ['Run status updated via workflow lifecycle handler.'],
      });
    }
