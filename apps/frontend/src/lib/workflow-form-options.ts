/**
 * @fileoverview apps/frontend/src/lib/workflow-form-options.ts
 *
 * High-level purpose:
...
const createSvgIcon = (paths: string[]): JSX.Element => {
  if (typeof document === "undefined") return null as unknown as JSX.Element;
...
const Icon = (d: string) => () => createSvgIcon([d]);
const IconMulti = (paths: string[]) => () => createSvgIcon(paths);
